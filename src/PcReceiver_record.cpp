...
std::thread RecordReceiverThread(std::atomic_bool& stop_sig, size_t batch_size, const std::string &record_filename, bool use_zstd, Eigen::MatrixXd& points,
    std::mutex& mutex, std::condition_variable& cv, std::atomic_bool& flag)
{
    flag = false;
    return std::thread([&, batch_size, record_filename, use_zstd]() {
        const size_t times = batch_size / 96;
        size_t idx = 0;
            ...
            case 1:
                    ...
                    ; // fmt::print("data_type: Point Cloud Data1\n");
                {
                    ...
                    for (int i = 0; i < 96; ++i) {
                        ...
                        points(idx * 96 + i, 0) = data->at(i).x.value();
                        points(idx * 96 + i, 1) = data->at(i).y.value();
                        points(idx * 96 + i, 2) = data->at(i).z.value();
                    }
